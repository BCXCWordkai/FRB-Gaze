...
|-- utils.py               # Loss, metric, TTA, and seed utilities
|-- requirements.txt
|-- LICENSE
`-- CITATION.cff
```
...
python test.py \
  ...
  --checkpoint checkpoints/MPIIFaceGaze/FRB_Gaze_best_ema.pth \
  --batch-size 128
```
...
python test.py \
  ...
  --checkpoint checkpoints/RT-GENE/FRB_Gaze_best_ema.pth \
  --batch-size 128
```
...
python test.py \
  ...
  --checkpoint checkpoints/ETH-XGaze/FRB_Gaze_best_ema.pth \
  --batch-size 128
```
...
If test-time horizontal flip augmentation is used in an experiment, add `--use-tta` during evaluation:
...
Use the same setting consistently when comparing results.
...
- Exponential moving average weights are saved as `FRB_Gaze_best_ema.pth`.
