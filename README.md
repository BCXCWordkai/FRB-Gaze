...
## Trained FRB-Gaze Checkpoint

We provide a trained FRB-Gaze checkpoint on ETH-XGaze for direct evaluation and reproducibility checking.

Recommended GitHub Release asset:

| Dataset | Checkpoint file | Size | SHA256 | Expected mean angular error |
| --- | --- | --- | --- | --- |
| ETH-XGaze | `FRB_Gaze_ETH-XGaze_best_ema.pth` | 90.7 MB | `273195126C45BC1C668B0098FAC6A93C34095646848ED4D433CB36BB4BFFE533` | 3.26 degrees |

After downloading the checkpoint, place it under `checkpoints/ETH-XGaze/`:

```text
checkpoints/
`-- ETH-XGaze/
    `-- FRB_Gaze_ETH-XGaze_best_ema.pth
```
...
The checkpoint can be attached to a GitHub Release or archived in a research repository such as Zenodo. The SHA256 checksum above can be used to verify the downloaded file.
