...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if norm_mae < best_norm_mae:
            ...
            torch.save(model.state_dict(), args.save_dir / f"{args.ablation_id}_best_norm.pth")
            improved = True
        if ema_mae < best_ema_mae:
            best_ema_mae = ema_mae
            torch.save(ema.module.state_dict(), args.save_dir / f"{args.ablation_id}_best_ema.pth")
            improved = True
        if epoch % 5 == 0:
            torch.save(
                {
                ...
                args.save_dir / f"{args.ablation_id}_epoch_{epoch}.pth",
            )
